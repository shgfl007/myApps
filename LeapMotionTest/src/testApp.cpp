	...
	if( leap.isFrameNew() && hands.size() ){
		...
		for(int i = 0; i < hands.size(); i++){
            for(int j = 0; j < hands[i].fingers().count(); j++){
				...
				pt = leap.getMappedofPoint( finger.tipPosition() );finger.tipVelocity();
                /*ofLog(OF_LOG_NOTICE, "the x position is " + ofToString(pt.x));
                ofLog(OF_LOG_NOTICE, "the y position is " + ofToString(pt.y));
                ofLog(OF_LOG_NOTICE, "the z position is " + ofToString(pt.z));*/
