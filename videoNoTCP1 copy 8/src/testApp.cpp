...
void testApp::setup(){
    ...
    //----------------------------- TCP -----------------------------------------
    isConnected = tcpClient.setup("127.0.0.1", 14999);
...
void testApp::update(){
    ...
    if (cv == numbofvideo) {
        //if (!isSend) {
            msgTx = "1";
            //tcpClient.send(msgTx);
            //ofLogNotice("isFull","1");
            //ofLogNotice("message",msgTx);
            //isSend=true;
        //}
         //serial_number_of_client[/p]is_all_video_triggered
    ...
    // }
    leap.markFrameAsOld();
    //ofLogNotice("sent message", msgTx);
